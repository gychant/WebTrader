...
SESSION_TOKEN_TYPE_KEY = "token_type"


def add_session(key, token_type=None):
    ...
    SESSION_DICT[key] = sess
    if key != "default":
        sess.headers["Authorization"] = "{0} {1}".format(token_type, key)
...
def SESSION():
    if SESSION_KEY not in session \
        or session[SESSION_KEY] is None:
        if "default" not in SESSION_DICT:
            add_session("default")
        return SESSION_DICT["default"]
    # Add the current session to all workers
    if session[SESSION_KEY] not in SESSION_DICT:
        add_session(session[SESSION_KEY], session[SESSION_TOKEN_TYPE_KEY])
    return SESSION_DICT.get(session[SESSION_KEY])
...
def filter_data(data, info):
    ...
    if info is not None:
        if info in compareDict and type(data) == list:
            ...
            logger.error(error_argument_not_key_in_dictionary(info))
            return noneType
    else:
        return data
...
def request_get(url, dataType='regular', payload=None, jsonify_data=True):
    ...
    if dataType == 'results':
        try:
            ...
            logger.error("{0} is not a key in the dictionary".format(message))
            return [None]
        ...
        while nextData['next']:
            try:
                ...
                logger.error('Additional pages exist but could not be loaded.')
                return data
            logger.info('Loading page {} ...'.format(counter))
            counter += 1
...
def request_post(url, payload=None, timeout=16, json=False, jsonify_data=True):
    ...
    try:
        ...
        logger.error("Error in request_post: {0}".format(message))
...
def request_delete(url):
    ...
    try:
        ...
        data = None
        logger.error("Error in request_delete: {0}".format(message))
