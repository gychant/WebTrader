...
def get_quotes(inputSymbols, info=None):
    ...
    for count, item in enumerate(data):
        if item is None:
            logger.error(error_ticker_does_not_exist(symbols[count]))
...
def get_instruments_by_symbols(inputSymbols, info=None):
    ...
    for item in symbols:
        ...
        if itemData:
            ...
            logger.error(error_ticker_does_not_exist(item))
    ...
    for item in quote:
        if item:
            if priceType == 'ask_price':
                ...
                if priceType:
                    logger.warn('WARNING: priceType should be "ask_price" or "bid_price". You entered "{0}"'.format(priceType))
...
def find_instrument_data(query):
    ...
    if len(data) == 0:
        logger.info('No results found for that keyword')
        return([None])
    else:
        logger.info('Found ' + str(len(data)) + ' results')
        return(data)
...
def get_stock_historicals(inputSymbols, interval='hour', span='week', bounds='regular', info=None):
    ...
    if interval not in interval_check:
        logger.error('Interval must be "5minute","10minute","hour","day",or "week"')
        return([None])
