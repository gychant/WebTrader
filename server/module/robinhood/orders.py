...
def find_stock_orders(**arguments):
    ...
    for item in data:
        for i, (key, value) in enumerate(arguments.items()):
            if key not in item:
                logger.error(error_argument_not_key_in_dictionary(key))
                return([None])
            if value != item[key]:
                break
            if i == stop:
...
def cancel_stock_order(orderID):
    ...
    if data:
        logger.info('Order '+str(orderID)+' cancelled')
    return(data)
...
def cancel_all_stock_orders():
    ...
    logger.info('All Stock Orders Cancelled')
    return(data)
...
def cancel_all_option_orders():
    ...
    logger.info('All Option Orders Cancelled')
    return(data)
...
def cancel_all_crypto_orders():
    ...
    logger.info('All Crypto Orders Cancelled')
    return(data)
...
def order_buy_fractional_by_price(symbol, amountInDollars, timeInForce='gfd', extendedHours=False, jsonify=True):
    ...
    if amountInDollars < 1:
        logger.error("Fractional share price should meet minimum 1.00.")
        return None
